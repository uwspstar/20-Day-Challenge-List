...
Here's an answer using your provided template for **树的遍历 (Tree Traversal)** methods including **前序遍历 (Pre-order)**, **中序遍历 (In-order)**, **后序遍历 (Post-order)**, and **层次遍历 (Level-order)**:
...
1. **LeetCode 题号和题目名称**
   ...
   **题目分析**：
   - 分析题目解法，讨论使用的算法和数据结构，并解释为什么使用这些方法。涉及二叉树的遍历、二叉搜索树的特性、堆树的结构等。

   **代码实现**：
   - 逐行中文注释代码，解释每个变量和逻辑的用途，便于理解。

   **时间和空间复杂度分析**：
   - **时间复杂度**：O(n) / O(log n) / O(1) 等，并详细说明。
   - **空间复杂度**：解释辅助数据结构、递归调用栈等空间消耗。

---

接下来，我会按照上述模板，逐步详细讲解30道LeetCode树相关的题目，每次展示5道，内容包括逐行中文注释代码及复杂度分析。我们先从最常见的树题目开始。

---

### 1. LeetCode 104: Maximum Depth of Binary Tree（最大二叉树深度）
